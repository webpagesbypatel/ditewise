...
function AppLogo() {
  ...
  return (
    <Link href="/dashboard" className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-primary/5 transition-colors duration-300">
      ...
      {state === 'expanded' && <h1 className="text-xl font-bold text-gradient">{APP_NAME}</h1>}
    </Link>
  )
}
  ...
  return (
    <SidebarMenuItem>
      <SidebarMenuButton
        ...
        className={cn(
          "justify-start rounded-lg transition-all duration-300 hover:shadow-sm",
...
export function AppSidebar() {
  return (
    <Sidebar variant="sidebar" collapsible="icon" side="left" className="border-r border-gray-200 bg-white/80 backdrop-blur-sm">
      ...
      <SidebarContent asChild>
        ...
        <SidebarGroup>
          <SidebarGroupLabel className="px-4 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">AI Tools</SidebarGroupLabel>
          <div className="p-2">
            <SidebarMenu className="gap-1">
      ...
      <SidebarFooter className="flex flex-col gap-2 p-4 border-t border-gray-100">
         <SidebarMenu>
